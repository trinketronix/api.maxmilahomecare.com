...
class Sender
{
     ...
     * If empty, Sender attempts to find one with, in order,
     ...
     * Default is $Hostname. If $Hostname is empty, Sender attempts to find
    ...
    /**
     * An implementation of the Sender OAuthTokenProvider interface.
     *
     ...
     * By default Sender will use `echo` if run from a `cli` or `cli-server` SAPI, `html` otherwise.
     ...
     * @var bool
     *
     * @deprecated 6.0.0 Sender isn't a mailing list manager!
     */
     ...
     * Options: An empty string for Sender default, whitespace/null for none, or a string to use.
     *
     * @var string|null
     */
    public $XMailer = '';
     ...
     *
     * @see Sender::$Debugoutput
     * @see Sender::$SMTPDebug
     *
     * @param string $str
     */
    ...
    /**
     * Add an address to one of the recipient arrays or to the ReplyTo array. Because Sender
     * can't validate addresses with an IDN without knowing the Sender::$CharSet (that can still
    ...
    protected function addOrEnqueueAnAddress($kind, $address, $name)
    {
        ...
        //Enqueue addresses with IDN until we know the Sender::$CharSet.
     ...
     *
     * ```php
     * Sender::validateAddress('user@example.com', function($address) {
     *     return (strpos($address, '@') !== false);
     * });
     * ```
     *
     * You can also set the Sender::$validator static to a callable, allowing built-in methods to use your validator.
     *
     ...
     * Important: Address must be passed in same encoding as currently set in Sender::$CharSet.
    ...
    protected function sendmailSend($header, $body)
    {
        ...
        if (!empty($this->Sender) && static::validateAddress($this->Sender) && self::isShellSafe($this->Sender)) {
            ...
            //<https://github.com/Sender/Sender/issues/2298>), and
     ...
     *
     * @see https://github.com/Sender/Sender/issues/924 CVE-2016-10045 bug report
     *
     ...
     *
     * @see Sender::setSMTPInstance() to use a different class.
     *
     * @uses \Sender\Sender\SMTP
     *
     * @param string $header The message headers
     * @param string $body   The message body
     *
    ...
    public function createHeader()
    {
        ...
        if ('' === $this->XMailer) {
            ...
            $result .= $this->headerLine(
                'X-Mailer',
                'Sender ' . self::VERSION . ' (https://github.com/Sender/Sender)'
            );
     ...
     * Sender only supports some preset message types, not arbitrary MIME structures.
     */
     ...
     * Explicitly *does not* support passing URLs; Sender is not an HTTP client.
     ...
     *                            Sender will attempt to set a MIME type for the attachment.
    ...
    protected function lang($key)
    {
        ...
        if (array_key_exists($key, $this->language)) {
            if ('smtp_connect_failed' === $key) {
                ...
                //but it's usually not Sender's fault.
                return $this->language[$key] . ' https://github.com/Sender/Sender/wiki/Troubleshooting';
            }
