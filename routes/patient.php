...
if (isset($app)) {
    ...
    $app->post('/patient/{patientId:[0-9]+}/upload/photo', [$patient, 'uploadPhoto']);

    // Update photo routes
    // For updating own photo (backward compatibility)
    // POST, not PUT: PHP only parses multipart bodies for POST requests
    $app->post('/patient/update/photo', [$patient, 'updatePhoto']);

    // For updating photo for a specific user (admin/manager)
    $app->post('/patient/{patientId:[0-9]+}/update/photo', [$patient, 'updatePhoto']);
